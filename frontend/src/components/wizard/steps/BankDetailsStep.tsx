...
export function BankDetailsStep({ form }: BankDetailsStepProps) {
  ...
  // Bank form section
  const BankForm = ({ prefix = '', label = '' }: { prefix?: string; label?: string }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <BankForm label={isJoint ? "Primary Applicant's Bank Details" : undefined} />

      {/* Joint Applicant Bank Details */}
      {isJoint && (
        <BankForm prefix="joint" label="Joint Applicant's Bank Details" />
      )}
    </div>
  );
}
